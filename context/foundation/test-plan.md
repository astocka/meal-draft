...
>
> Last updated: 2026-06-06 (rollout order: data isolation first; Risk #6 guidance backported from data-isolation research)
...
- Provider/platform: GitHub Actions CI (lint + build only, no tests) — quality-gate target for Phase 4; checked: 2026-06-06
...
| unit + integration | local + CI | required after §3 Phase 1 (data isolation) | logic regressions, RLS isolation, schema drift |
| workerd smoke (build + preview) | local pre-deploy | required after §3 Phase 4 | Node vs workerd runtime mismatch |
| e2e on critical flows | CI on PR | planned | deferred — integration covers v1 critical paths |
...
TBD — see §3 Phase 4 for Try another in-flight race / loading pattern (Risk #3).
