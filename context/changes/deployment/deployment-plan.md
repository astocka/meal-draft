...
The scaffolding is mostly in place:

- `@astrojs/cloudflare@^13.5.0` adapter configured in `astro.config.mjs`
- `wrangler@^4.90.0` available as a devDependency
- `wrangler.jsonc` exists with `nodejs_compat`, `compatibility_date: 2026-05-08`, observability enabled, and the correct Astro entrypoint
- CI (`.github/workflows/ci.yml`) runs lint + build -- **no deploy step (stays that way)**
- Secrets (`SUPABASE_URL`, `SUPABASE_KEY`) injected as env vars during CI build but **not set on Cloudflare yet**
- No `.dev.vars.example` template exists (only `.env.example`)
- No `deploy` npm script exists
- Project name in `wrangler.jsonc` is still `"10x-astro-starter"` (needs renaming to `meal-draft`)

**Pre-existing CI bug:** `.github/workflows/ci.yml` uses `npm ci` and `cache: npm`, but the project uses **pnpm** (`pnpm-lock.yaml`, no `package-lock.json`). `npm ci` will fail without `package-lock.json`. This must be fixed before deployment (covered in Phase 0F).
...
- **0D.2** Ensure pnpm is installed globally:
  ```bash
  pnpm --version
  ```
  If not installed: `npm install -g pnpm` (or use `corepack enable` with Node 22).
- **0D.3** Install dependencies:
  ```bash
  pnpm install
  ```
- **0D.4** Verify the project builds successfully against the Cloudflare adapter:
  ```bash
  pnpm run build
  ```
...
- **0E.1** Copy `.env.example` to `.env` (for `astro dev` which runs on Node.js):
  ```bash
  cp .env.example .env
  ```
  Fill in `SUPABASE_URL` and `SUPABASE_KEY` from step 0C.3.
- **0E.2** Copy `.env.example` to `.dev.vars` (for `wrangler dev` / `astro preview` which runs on workerd):
  ```bash
...
- **0F.1** Update `.github/workflows/ci.yml`:
  ```yaml
  name: CI
...
- **2.1** Add convenience scripts to `package.json`:
  ```json
...
2. Confirm `compatibility_date` is 2025 or later (already `2026-05-08`)
...
- **5.1** Bump `compatibility_date` in `wrangler.jsonc` to `"2026-05-25"` (today). Document a quarterly bump cadence in `AGENTS.md`.
- **5.2** Add a Cloudflare operations section to `AGENTS.md`:
  ```
  ## Cloudflare
...
- **7.1** Update `AGENTS.md` with the Cloudflare section from Phase 5.2 plus deploy commands
- **7.2** Add deploy instructions to the project README, covering:
...
- `AGENTS.md` -- add Cloudflare operations section
...
flowchart LR
  subgraph local [Local Development]
    DEV["astro dev<br/>(Node.js + platformProxy)"]
